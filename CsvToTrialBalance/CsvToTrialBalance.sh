...
../misc/dw-script-start.py 12 "$username3" "$password3" "$MYSQL_HOST" "$MYSQL_PORT" 1>&4 2>&3 
TIMEFORMAT='%R';time ./CsvToTrialBalanceMultiLevel.py "$pcn" "$username3" "$password3" "$MYSQL_HOST" "$MYSQL_PORT" "$start_period" "$end_period" "$email" 1>&4 2>&3
result=$?
if [[ $result -eq 0 ]]
then # if/then branch
    ../misc/dw-script-end.py 12 0 "$username3" "$password3" "$MYSQL_HOST" "$MYSQL_PORT" 1>&4 2>&3
else
  ../misc/dw-script-end.py 12 1 "$username3" "$password3" "$MYSQL_HOST" "$MYSQL_PORT" 1>&4 2>&3
fi
} 2>&6 
