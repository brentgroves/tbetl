...
printf "\nsed-script-start-py build: $build" 
printf "\nsed-script-start-py mysql_host: $mysql_host" 
printf "\nsed-script-start-py mysql_port: $mysql_port" 
printf "\nsed-script-start-py azure_dw: $azure_dw" 
printf "\nsed-script-start-py file: $file" 

if [[ $build == 'dev' ]]
then
  printf '\ndev build'
  sed -e '/%DEV%/s/%DEV%/  /' tmp-script-start.py | \
  ...
  printf '\nprod build'
  sed -e '/%DEV%/s/^/#/' tmp-script-start.py | \
  sed -e "/%MYSQL_HOST%/s/%MYSQL_HOST%/$mysql_host/" | \
  sed -e "/%MYSQL_PORT%/s/%MYSQL_PORT%/$mysql_port/" | \
  sed -e "/%AZURE_DW%/s/%AZURE_DW%/$azure_dw/" | \
